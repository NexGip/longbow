...
const stillActive = (
  ...
  const capitalDate =  new Date(investment.activeDate).getTime() +
  ...
  for (let i = 0; i < investments.length; i++) {
    ...
    if (investment.capitalPaid && investment.expired) invalid.push(investment);
      ...
      if (
        ...
        let amount =  interestBalance + capital;
...
export const calUserBalance = async (
  ...
  investments.forEach((investment) => {
    ...
    const activeDate = new Date(
      investment.activeDate
    );
    const todayVal = today.getTime();
    const exVal =
      activeDate.getTime() +
      ( plan.duration) * dayInMilliSec;
    const daysLeft = exVal > todayVal ? (exVal - todayVal) / dayInMilliSec : 0;
    const daysGone =
      ( plan.duration) - daysLeft;
    const centage =
      daysGone / ( plan.duration);
